...
struct circleGeometry{
    ...
    pair<Point,Point> lineCircleIntersectionPoints(Point center, double r, Point P1, Point P2){
        ...
        double C = square(b1) + square(b2) - square(r);
    ...
    double circleCircleIntersectionArea(Point P0, double R0, Point P1, double R1) {
        double d = dist(P0, P1);
        if(R0 + R1 <= d) return 0;
        if(R0 >= R1 && (R0 - R1) >= d) return pi * square(R1);
        if(R0 <= R1 && (R1 - R0) >= d) return pi * square(R0);

        double CAD = 2.0 * acos((square(R0) + square(d) - square(R1)) / (2.0 * R0 * d));
        double CBD = 2.0 * acos((square(R1) + square(d) - square(R0)) / (2.0 * R1 * d));

        //double A1 = (0.5 * square(R0) * CAD) - (0.5 * square(R0) * sin(CAD));
        double A1 = 0.5 * square(R0) * (CAD - sin(CAD));
        //double A2 = (0.5 * square(R1) * CBD) - (0.5 * square(R1) * sin(CBD));
        double A2 = 0.5 * square(R1) * (CBD - sin(CBD));
        return (A1 + A2);
    }
    ...
    void circleCircleIntersectionPoints(Point P0, double R0, Point P1, double R1) {
        ...
        if(d > R0 + R1){
            ...
            double a = (square(R0) - square(R1) + square(d))/(2.0 * d);
            double b = d - a;
            double h = sqrt(square(R0) - square(a));
